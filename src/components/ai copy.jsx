      ...
      {sorted.length === 0 ? (
        ...
        <>
          <p className="ai-mode-count">
            {sorted.length} result{sorted.length !== 1 ? 's' : ''} — click title to open
          </p>
          <div className="ai-mode-nav">
            <button
              type="button"
              className="btn-ghost btn-sm"
              disabled={viewIndex === 0}
              onClick={() => showAt(viewIndex - 1)}
            >
              ← Prev
            </button>
            <span className="ai-mode-pos">
              {viewIndex + 1} / {sorted.length}
            </span>
            <button
              type="button"
              className="btn-ghost btn-sm"
              disabled={viewIndex >= sorted.length - 1}
              onClick={() => showAt(viewIndex + 1)}
            >
              Next →
            </button>
          ...
          <ul className="activity-list ai-list">
            {current && (
              <AiResponseCard
                ...
                item={current}
                questions={questions}
                expanded={openCurrent}
                onToggle={() => onToggle(openCurrent ? null : current.id)}
              />
            )}
          </ul>
        </>
